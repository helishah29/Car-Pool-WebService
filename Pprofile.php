...
<?php
	if(isset($_REQUEST["upd"]))
	{
		...
		$result=mysql_query("select * from passenger where (uname='$uname' or email='$mail' or ph=$ph) and uname!='".$_SESSION['Access']."'");
		
		if(mysql_num_rows($result)==0)
		{
			mysql_query("update passenger set uname='$uname',email='$mail',ph=$ph where uname='".$_SESSION['Access']."'");
		...
		else
		{
			...
			$row=mysql_fetch_assoc($result);
				...
				$result=mysql_query("select * from passenger where email='$em' and uname!='".$_SESSION['Access']."'");
				if(mysql_num_rows($result)!=0)
				{
					echo "<h4 align=center style=color:teal>Email Already Exists</h4></center>";
				}
				
				$result=mysql_query("select ph from passenger where ph=$p and uname!='".$_SESSION['Access']."'");
