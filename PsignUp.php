...
/*function checkPass()
{
    ...
    if(rex.test(pass))
    {
        if(pass.equals(f1.uname.value))
            chkpass.innerHTML="Password Cant Be Same As UserName";
...
<table align="center">
    ...
    <tr><td>E-mail:</td><td><input type="email" name="mail" onblur="checkm()"></td><td><span id="mailchk"></span><td></tr>
...
<?php
	...
	if(isset($_REQUEST["sub"]))
	{
		...
		{
			mysql_query("insert into passenger(Name,uname,password,age,ph,email,sex,Addr) values('$Name','$uname','$pass',$age,$ph,'$mail','$sex','$add')");
		...
		else
		{
			...
			$row=mysql_fetch_assoc($result);
				...
				$result=mysql_query("select * from passenger where uname='$un'");
				if(mysql_num_rows($result)!=0)
				{
					echo " <br> <h4 align=center style=color:teal>UserName Already Exists</h4>";
				}

				$result=mysql_query("select * from passenger where email='$em'");
				if(mysql_num_rows($result)!=0)
				{
					echo "<h4 align=center style=color:teal>Email Already Exists</h4></center>";
				}
				
				$result=mysql_query("select * from passenger where ph=$p");
				if(mysql_num_rows($result)!=0)
				{
